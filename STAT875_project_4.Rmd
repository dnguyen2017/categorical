...
```{r}
# get proportiona of each win type at each drop position
plinko <- plinko %>% group_by(drop) %>% mutate(total_count = sum(count), win_prop = count/total_count)

pred1 %>% pivot_longer(cols = 2:6, names_to = "win", values_to = "probability") %>%
  ggplot() +
  geom_line(aes(x = drop, y = probability, col = win)) +
  #geom_point(data = plinko, mapping = aes(x = drop, y = win_prop, col = win, size = count)) +
...
```{r}
#plinko$win_ordinal <- factor(plinko$win, levels = c(4, 1, 2, 3, 5))
...
pred2 <- cbind(pred2, predict(mod.fit2, 
                     newdata = data.frame(drop = 1:5), 
                     type = "probs"))
# pihat_labels <- c("drop", "$\\hat \\pi_4$", "$\\hat\\pi_1$", "$\\hat\\pi_2$",
#                   "$\\hat\\pi_3$", "$\\hat\\pi_5$")
...
pred2 %>% pivot_longer(cols = 2:6, names_to = "win", values_to = "probability") %>%
  ggplot() +
  geom_line(aes(x = drop, y = probability, col = win)) +
  #geom_point(data = plinko, mapping = aes(x = drop, y = win_prop, col = win_ordinal, size = count)) +
  labs(title = "Estimated probability for each win type",
       y = expression(hat(pi[j]))) +
  theme_bw()
```
