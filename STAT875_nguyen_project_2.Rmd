...
The estimated odds of a success are $e^{\hat \beta_1}=$ `r ORhat_fout` as large when the flagstick is taken out compared to when the flagstick is left in and all other variables are fixed.

The estimated odds of a success are $e^{-\hat \beta_2}=$ `r ORhat_bs` as large for every one-unit decrease in ball speed when all other variables are fixed.

The estimated odds of a success are $e^{-\hat \beta_3}=$ `r ORhat_el` as large for every one-unit decrease in entry line when all other variables are fixed.
