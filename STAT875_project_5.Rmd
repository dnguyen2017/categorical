...
pred_plot <-
  cbind(pred, data.frame(predict(mod.fit, newdata = pred, type = "link", se.fit = TRUE))) %>%
    mutate(mu = exp(fit),
...
Anova(mod.fit.interaction, test = "LR")
```

The likelihood ratio tests for the three interaction terms show that there is strong statistical evidence of a (mean tree height) x (number of trees) and (mean tree height) x (canopy cover) interactions based on the small p-values. These interactions should be included in the model.

There is weak evidence that there may be an effect of the (canopy cover) x (number of trees) interaction on the mean number of stripped cones. This interaction term could potentially be included in the model if there was some biological justification for it.